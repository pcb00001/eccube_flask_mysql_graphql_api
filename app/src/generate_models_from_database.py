...
def buildIndex(column_name, table_name):
    cursor_statistics = connection.cursor(MySQLdb.cursors.DictCursor)
    cursor_statistics.execute(query_check_index.format(column_name, table_name))
...
def buildModel(table_name, cursor):
    ...
    for (column_name, data_type, character_maximum_length, numeric_precision, numeric_scale, is_nullable, extra,
        ...
        cursor_key_column_usage = connection.cursor(MySQLdb.cursors.DictCursor)  # get the cursor
        cursor_key_column_usage.execute(query_constraint.format(column_name, table_name))
...
def outModelFile(model_name, content_model):
    f = open("{0}/model_{1}.py".format(config.model_folder, model_name), "w+")
...
for (table_name,) in cursor:
    if len(config.list_tables) == 0 or table_name in config.list_tables:
        cursor.execute(query_column_info.format(table_name))
        outModelFile(table_name, buildModel(table_name, cursor))
